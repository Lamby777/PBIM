// ==JavaScript==
const NAME = "PBIM (PianoBot IMproved)";
const NAMESPACE = "https://dexie.tk/";
const VERSION = "1.0.0";
const DESCRIPTION = "Plays MIDI files! (+ extras)";
const AUTHOR = "Dexie";
const INCLUDE = [/^https?:\/\/www\.multiplayerpiano\.com*/g,
                 /^https?:\/\/multiplayerpiano\.(com|net)*/g,
                 /^https?:\/\/mppclone\.com*/g];
const DOWNLOAD_URL = "https://raw.githubusercontent.com/Lamby777/PBIM/master/index.js";
const MIDIPlayerJS_URL = "https://raw.githubusercontent.com/grimmdude/MidiPlayerJS/master/browser/midiplayer.js";

// HELL YEAH, FELLOW GNU GPL USER
...
var PlayerSet = setInterval(function() {
    if (typeof(MidiPlayer) === "undefined" || MidiPlayer === null) return;
	...
	var useCorsUrl = function(url) {
		...
		if (url.indexOf(CORS_PROXY) == -1) newUrl = CORS_PROXY + url.replace(/(^\w+:|^)\/\//, '');
		return newUrl;
	}
		...
		fetch(url, {
			...
			if (corsUrl != null) {
				...
				fetch(corsUrl, {
					...
					console.error("CORS proxy couldn't get the file:", error);
					stopLoadingMusic();
					clearInterval(downloading);
					callback(null);
				});
